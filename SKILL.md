...
把正式发行版的 PH 落到尚未接入的 Git 仓库：空仓库安装脚手架，存量项目用旧内容接入（已有正文优先引用，不复制第二套），并在初始化会话逐项补全文档；也可只检查 / 修复适配层。已接入项目的版本合并走 `ph-merge-update`，不要用 `init --apply` 覆盖定制。
...
正式源只有 `https://github.com/chenweixuanJokes/ph-init.git`。`latest` 取数值最大的稳定 tag（排除预发布与非版本标签），并固定到该 tag 的 commit。本批版本为 `1.1.4`（Schema `1.1.1`，十个必需 Skill）。Schema 与发布版本独立，本批不改 Schema。
...
```text
python3 <ph-init-root>/scripts/ph_release.py prepare --version latest|1.1.4 --repo <target>
```
...
`--adopt-plan` 契约（仅当目标尚无 `.agents/ph.json`；已有项目升正式版走 `ph-merge-update`）：
