...
正式源只有 `https://github.com/chenweixuanJokes/ph-init.git`。`latest` 取数值最大的稳定 tag（排除预发布与非版本标签），并固定到该 tag 的 commit。本批版本为 `1.1.6`（Schema `1.1.1`，十个必需 Skill）。Schema 与发布版本独立，本批不改 Schema。
