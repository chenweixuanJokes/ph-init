...
description: 将 PH 管理的任务 worktree 按“验证、受控提交、合并回进入时记录的源工作区、再次验证”交付，并在另行取得用户确认后清理本次 worktree。只要用户说“退出 worktree”“收口并合回主目录”“完成这个并行任务”或调用 ph-worktree-exit，都必须使用本技能。普通提交、普通合并、非 PH 登记的 worktree、仅询问 Git 用法时不要使用；不得自动 push、删分支、stash、force 或清理其它会话。
---
