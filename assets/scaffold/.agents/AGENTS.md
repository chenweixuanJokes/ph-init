...
记忆库（参考，不是规范）在 `.agents/memory/README.md`。文档目录规则见 `docs/约束规范/工程规范/文档治理.md`。初始化补全按 `docs/约束规范/工程规范/初始化与文档补全.md` 取证；安全配置、构建发布与运维细则由工程规范 README 索引。只把已核实事实填入项目摘要，不把外部建议当已生效规则。
