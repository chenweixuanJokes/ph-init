...
def load_release(root: Path) -> dict:
    ...
    if len(skills) != 10 or len(set(skills)) != 10:
        raise CheckError("illegal release.json: required_skills must list 10 unique names")
    if skills[0] != "ph-init":
...
def chain_to(hops: list[dict], target: str) -> list[dict]:
    ...
    if target not in by_to:
        starts = sorted({hop["from_version"] for hop in hops} | {hop["to_version"] for hop in hops})
        previous = [ver for ver in starts if semver_tuple(ver) < semver_tuple(target)]
        if previous:
