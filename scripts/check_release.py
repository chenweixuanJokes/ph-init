...
# Strict kebab-case for the eleven distributed skills: lowercase alphanumeric
...
def load_release(root: Path) -> dict:
    ...
    if len(skills) != 11 or len(set(skills)) != 11:
        raise CheckError("illegal release.json: required_skills must list 11 unique names")
    if skills[0] != "ph-init":
...
def validate_skill(path: Path, expected_name: str) -> None:
    """Common strict subset shared by all eleven distributed skills.
