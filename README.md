...
不点名技能、直接描述任务时，符合技能触发条件的说法也会触发对应技能。普通检查和同步使用项目已安装的版本，不下载更新。升级会比较新旧内容，保留项目定制；与现有规则冲突的部分需要确认。

从 1.1.8 起，PH 只保留一个版本号。1.1.7 及更早的安装工具无法读取新版包，首次升级需要先取得新版工具。这一步不会自动完成；可以把下面的话交给助手：

> 从官方仓库的 v1.1.8 标签获取新版工具，放到项目之外的新目录，不覆盖已有入口。先用它准备 v1.1.8，再用新版工具准备最新正式版，检查这个项目的完整升级差异，确认后再写入。

升级完成后，使用项目内更新过的入口。用户级的旧副本没有被覆盖，若客户端仍优先加载它，需要调整客户端的技能入口。

## 下载

安装和升级从 [官方仓库](https://github.com/chenweixuanJokes/project-harness) 下载正式版本。开发分支不作为最新版；下载或校验失败时会停止。

官方仓库已由 `ph-init` 更名为 `project-harness`，旧地址会自动跳转到新仓库；已按旧地址安装的入口无需重装，也不用改技能目录名。
